...
## 7. best-hit filter
A best-hit filter is available `besthit_filter.py`. It chooses the best-scoring match subject for each query sequence in the csv file. For that, it takes a fasta file (queries used with BLASTn/x), the resulting output csv file and produces the same outputs as d-chimer at a given cycle (cf `section 2` of this document). 
